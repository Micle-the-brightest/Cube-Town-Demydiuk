...
public class ExplodeCubes : MonoBehaviour
{
    ...
    private bool _collisionSet;// дає змогу виконувати код один раз навіть якщ ще чомусь додати тег Cube 
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if(collision.gameObject.tag == "Cube" && !_collisionSet)
        {
            for (int i = collision.transform.childCount - 1; i >= 0; i--)
            {
                ...
                child.gameObject.GetComponent<Rigidbody>().AddExplosionForce(70f, Vector3.up, 5f);// розмах розкидування кубів
